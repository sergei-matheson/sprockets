module Sprockets
  class Preprocessor
    ...
    def require(source_file)
      ...
      if @options[:include_file_markers]
        concatenation.record(SourceLine.new(source_file, "// start: #{source_file.pathname}", 0))
      end
      ...
      if @options[:include_file_markers]
        concatenation.record(SourceLine.new(source_file, "// end: #{source_file.pathname}", 0))
      end
      #Tack a semi-colon on to the end of each file, to prevent inter-file breakage.
      concatenation.record(SourceLine.new(source_file, ';', 0))
    end
    ...
    def import_haml_from_source_line(source_line)
      ...
      if html_string 
        source_line.line= "#{prefix}#{html_string}"
